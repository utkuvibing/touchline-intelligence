...
**`distance_to_goal(x, y)`** — Euclidean distance to the goal centre `(120, 40)`, via `math.hypot`
rather than `sqrt(dx*dx + dy*dy)`: the stdlib implementation is correctly rounded and avoids the
intermediate overflow and underflow that squaring introduces.
