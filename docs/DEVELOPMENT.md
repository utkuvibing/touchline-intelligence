...
Everything a contributor needs after the quick start in [`README.md`](../README.md): the full command
matrix, the testing contract, ingestion behaviour, and the operational details of the data pipeline.
...
The local database exposes host port **5433**, not 5432, so it cannot collide with a PostgreSQL
already installed on the machine. `.env.example` matches this.
...
[`reproducibility/wp1_6_clean_rebuild.md`](reproducibility/wp1_6_clean_rebuild.md). The M1 technical
and stakeholder release artifact is [`releases/m1-data-foundation.md`](releases/m1-data-foundation.md).
...
[`../data/provenance/`](../data/provenance/), which is committed so another machine can prove it
received identical bytes. Downloaded files are cached under `data/statsbomb/<sha>/` (git-ignored);
...
or continuous tracking. Applied migration SQL is checksum protected. See [`SCHEMA.md`](SCHEMA.md)
and [ADR 0009](adr/0009-full-relational-event-and-lineup-scope.md).
...
Ten hand-written PostgreSQL queries live in [`../backend/sql/wp1_5/`](../backend/sql/wp1_5/). They
cover competition and match coverage, team results, home/away checks, descriptive shot prevalence,
...
[`analysis/wp1_5_sql_analysis_pack.md`](analysis/wp1_5_sql_analysis_pack.md). A narrow event-type
index improved one manual aggregate but was rejected: it added about 5.6 MiB plus write maintenance
and served no recurring or production workload. No speculative secondary index was retained.
...
- **Penalties and shootout kicks are excluded**, per [ADR 0004](adr/0004-cohort-scope-and-validation-design.md).
...
### `GET /shots?limit=&offset=&match_id=` (WP0.5)
...
- CI builds and checks; it does not deploy. Railway and Vercel deploy from their own GitHub
  integrations — see [`DEPLOYMENT.md`](DEPLOYMENT.md).
...
- No drift monitoring. A deliberate, recorded trade-off — see
  [ADR 0006](adr/0006-deployment-approach.md).
