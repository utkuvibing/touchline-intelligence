...
export function HomeView({ shots, rate, error }: HomeViewProps) {
  ...
  return (
    <main>
      ...
      <p>
        Football research and decision-support built on StatsBomb Open Data — a relational dataset,
        a calibrated shot-quality model, and an analyst interface.
      </p>
      ...
      <section aria-labelledby="map-heading">
        ...
        {error ? (
          ...
          <p role="alert">
            Could not load shots from the API: {error}. The map is empty because the data could not
            be fetched, not because no shots were taken.
          </p>
        ) : (
          <>
            <p>
      ...
      {rate && (
        <section aria-labelledby="rate-heading">
          ...
          <p>
            <strong>
              {(rate.conversion_rate * 100).toFixed(2)}% ({rate.goals} of {rate.shots})
            </strong>
          </p>
          <p>{rate.cohort}</p>
