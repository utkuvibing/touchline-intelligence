...
Prior full review: [`docs/research/statsbomb-open-data-terms-review-2026-08-01.md`](../docs/research/statsbomb-open-data-terms-review-2026-08-01.md).
This re-check verifies the same first-party sources at the project's pinned revision; nothing in
them was found to have changed at that revision.
...
| Deployed frontend ([live page](https://touchline-intelligence.vercel.app)) | Text | Live fetch 2026-08-25: "SOURCE AND TERMS — Data provided by StatsBomb through the StatsBomb Open Data repository"; "Not StatsBomb xG" limitations card; closed-publication-gate notice | Present |
...
2026-08-01 review) is met in the repository by logo + text together; the deployed interface
currently carries text attribution only, which the 2026-08-01 review accepted as satisfying the
requirement while logo asset availability was unresolved — availability is now resolved for
repository use.
...
2. **No data redistribution.** No raw JSON, database dumps, or derived data-bearing artifacts are
   published; committed fixtures remain synthetic.
...
- Added `scripts/check_docs_links.py` (manual; documented in `docs/DEVELOPMENT.md`) so the
  tracked-relative-link property this audit depends on stays checkable.

WP4.4 completion is **not** claimed by this report; independent review and the remaining milestone
