...
- **`poe migrate`.** Applying ordered migrations to the deployed database is a documented operator
  step (`docs/DEPLOYMENT.md` step 1) that changes structure, not application data. Blocking it as a
  side effect of protecting ingestion would break the release runbook. Assessed separately; a test
  records the decision so it cannot drift silently.
